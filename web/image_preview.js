...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name === "ImagePreviewNode") {
            ...
            // 递归获取上游节点链（最多10个节点）
            nodeType.prototype.getUpstreamChain = function(maxDepth = 10, currentDepth = 0, visited = new Set()) {
                ...
                const chain = [upstreamNode];
                
                // 递归获取更上游的节点（使用辅助函数避免依赖节点方法）
                const getNodeUpstream = (node) => {
                ...
                let currentNode = upstreamNode;
                let depth = currentDepth + 1;
                
                while (depth < maxDepth) {
                    const nextNode = getNodeUpstream(currentNode);
            ...
            nodeType.prototype.checkUpstreamNodes = function() {
                // 获取上游节点链（最多10个）
                const upstreamChain = this.getUpstreamChain(10);
                ...
                // 清理不再需要的监听器
                const currentNodeIds = new Set(upstreamChain.map(n => n.id));
                const oldNodeIds = Array.from(this.upstreamListeners.keys());
                oldNodeIds.forEach(nodeId => {
                    if (!currentNodeIds.has(nodeId)) {
                        // 从缓存中移除
                        unregisterPreviewNode(this, nodeId);
                        // 移除监听器
                        const listener = this.upstreamListeners.get(nodeId);
                        if (listener && listener.originalCallbacks) {
                        ...
                        this.upstreamListeners.delete(nodeId);
                    }
                });
                
                // 监听所有上游节点
                upstreamChain.forEach((upstreamNode, index) => {
                    // 如果已经监听了这个节点，跳过
                    ...
                    console.log(`[ImagePreview] 节点 ${this.id} 开始监听上游节点[${index}] ${upstreamNode.id} (${upstreamNode.type})`);
                ...
                // 更新最后的上游节点ID（用于兼容旧代码）
                if (upstreamChain.length > 0) {
                    this.lastUpstreamNodeId = upstreamChain[0].id;
                }
            };
                                ...
                                previewNodes.forEach(node => {
                                    if (node && !node.removed) {
                                        ...
                                        if (node.pendingBackendCall) {
                                            ...
                                            // 延迟验证，不影响即时响应
                                            setTimeout(() => {
                                                if (!node.isDragging && !node.removed) {
                                                    node.processInBackend(upstream, params, false);
                                                }
                                            }, 500); // 延迟500ms验证，不影响交互
                                        }
                                    }
            ...
            nodeType.prototype.onUpstreamWidgetChanged = function(upstreamNode, widgetName, value) {
                ...
                // 如果正在拖动，仅使用前端快速预览
                if (this.isDragging) {
                    // 如果有上游链，使用链中直接连接的节点进行快速预览
                    const targetNode = (this.upstreamChain && this.upstreamChain.length > 0) ? 
                        this.upstreamChain[this.upstreamChain.length - 1] : upstreamNode;
                    const params = this.extractNodeParams(targetNode);
            ...
            nodeType.prototype.processUpstreamNode = function(upstreamNode) {
                ...
                // 如果有上游链，处理整个链（从最上游到最下游）
                if (this.upstreamChain && this.upstreamChain.length > 0) {
                    console.log(`[ImagePreview] 处理上游节点链（共${this.upstreamChain.length}个节点）`);
                    
                    // 从最上游开始处理（反向顺序）
                    const chain = [...this.upstreamChain].reverse();
                    
                    // 先尝试使用后端API处理整个链
                    this.processChainInBackend(chain);
                    
                    // 如果后端处理失败，使用前端快速预览
                    // 前端预览只处理直接连接的节点（性能考虑）
                    if (chain.length > 0) {
                        const directNode = chain[0]; // 直接连接的节点
                        const nodeTypeName = directNode.type || directNode.comfyClass || directNode.title || "";
                        const params = this.extractNodeParams(directNode);
            ...
            // 处理上游节点链（从最上游到最下游依次处理）
            nodeType.prototype.processChainInBackend = async function(chain) {
                if (!this.originalImageData || !this.canvas || chain.length === 0) {
                    return;
                }
                
                // 检查参数是否变化（避免重复请求）
                const chainParams = chain.map(node => ({
                    id: node.id,
                    type: node.type || node.comfyClass || node.title || "",
                    params: this.extractNodeParams(node)
                }));
                const paramsStr = JSON.stringify(chainParams);
                ...
                // 优化：拖动时完全禁用后端处理
                if (this.isDragging) {
                    console.log('[ImagePreview] 拖动中：跳过后端处理，使用前端预览');
                    return;
                }
                
                console.log(`[ImagePreview] 参数已变化，准备调用后端API处理节点链`);
                ...
                this.backendThrottle = setTimeout(async () => {
                    try {
                        ...
                        console.log('[ImagePreview] 开始执行后端处理（节点链）');
                        await this.executeChainBackendProcess(chain);
            ...
            // 执行节点链的后端处理
            nodeType.prototype.executeChainBackendProcess = async function(chain) {
                if (!this.originalImageData || !this.canvas || chain.length === 0) {
                    return;
                }
                ...
                try {
                    ...
                    // 构建节点链信息（从最上游到最下游）
                    const chainInfo = chain.map(node => ({
                        id: node.id,
                        type: node.type || node.comfyClass || node.title || node.constructor?.name || "",
                        params: this.extractNodeParams(node)
                    }));
                    
                    console.log(`[ImagePreview] 准备调用后端API处理节点链:`, chainInfo.map(n => n.type));
                    
                    // 调用后端API（传递节点链信息）
                    const response = await api.fetchApi('/image_preview/process_chain', {
                        method: 'POST',
                        headers: {
                    ...
                    if (!response.ok) {
                        // 如果后端不支持链处理，回退到只处理直接连接的节点
                        console.warn('[ImagePreview] 后端不支持链处理，回退到单节点处理');
                        const directNode = chain[chain.length - 1]; // 直接连接的节点
                        await this.executeBackendProcess(directNode, this.extractNodeParams(directNode));
                        return;
                    }
                    ...
                    if (result.success && result.image_data) {
                        ...
                        img.onload = () => {
                            ...
                            const displayWidth = result.width || originalWidth;
                            const displayHeight = result.height || originalHeight;
                            this.canvas.width = displayWidth;
                            this.canvas.height = displayHeight;
                            ctx.drawImage(img, 0, 0);
                            console.log('[ImagePreview] 预览已更新（节点链处理）');
                        };
                        img.onerror = () => {
                            console.error('[ImagePreview] 图像加载失败');
                            this.updatePreview();
                        };
                    ...
                    // 出错时回退到只处理直接连接的节点
                    if (chain.length > 0) {
                        const directNode = chain[chain.length - 1];
                        await this.executeBackendProcess(directNode, this.extractNodeParams(directNode));
                    } else {
                        this.updatePreview();
                    }
            ...
            nodeType.prototype.processInBackend = async function(upstreamNode, params, isDragging = false) {
                ...
                // 优化：拖动时完全禁用后端处理，仅使用前端预览
                if (this.isDragging || isDragging) {
                    console.log('[ImagePreview] 拖动中：跳过后端处理，使用前端预览');
                    return; // 拖动时直接返回，不执行后端处理
                }
                
                console.log(`[ImagePreview] 参数已变化，准备调用后端API处理`);
