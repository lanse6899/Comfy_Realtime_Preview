...
class ImagePreviewNode:
    ...
    def preview(self, image, unique_id):
        ...
        try:
            ...
            # 将图像转换为base64格式发送到前端（优化：使用JPEG格式提高速度）
            ...
            try:
                ...
                if node_id and node_id in node_data:
                    del node_data[node_id]
                return (image,)
    ...
    try:
        ...
        try:
            ...
            if processed_tensor is None:
                ...
                # 应用所有参数（完全无限制 - 所有数值参数都应用所有图像处理方法）
                # 不对参数名称和数值范围做任何限制，所有参数都尝试应用所有可能的图像处理效果
                ...
                for param_name, param_value in params.items():
                    try:
                        ...
                        if abs(value) > 0.0001:
                            ...
                            # 归一化因子到合理范围
                            if abs(factor) > 10:
                                factor = 10 if factor > 0 else -10
                            if 0.1 <= abs(factor) <= 10.0:
                                processed_img = processed_img * factor
                        
                        # 2. 偏移变换（对比度类）
                        if abs(value) > 0.01:
                            offset = value * 0.3  # 缩放偏移量
                            processed_img = processed_img + offset
                        
                        # 3. HSV空间变换（饱和度/色相类）- 对所有参数都尝试
                        if use_cv2 and abs(value) > 0.01:
                            try:
                                hsv = cv2.cvtColor(np.clip(processed_img, 0, 255).astype(np.uint8), cv2.COLOR_RGB2HSV).astype(np.float32)
                                # 饱和度调整
                        ...
                        if use_cv2 and abs(value) > 0.1:
                            try:
                                ...
                                if abs(value) > 1.0:
                                    # 锐化
                                    kernel = np.array([[0, -0.3, 0], [-0.3, 2.2, -0.3], [0, -0.3, 0]]) * min(abs(value) * 0.05, 0.5)
                                else:
                                    # 轻度锐化
                                    kernel = np.array([[0, -0.1, 0], [-0.1, 1.4, -0.1], [0, -0.1, 0]]) * abs(value) * 0.1
                        ...
                        if abs(value) > 0.01:
                            # 根据参数索引选择通道（伪随机但确定）
                            channel_idx = hash(param_name) % 3
                            channel_factor = 1.0 + value * 0.1
                            processed_img[:, :, channel_idx] = processed_img[:, :, channel_idx] * channel_factor
                        
                        # 6. 基于亮度的区域调整（高光/阴影效果）
                        if abs(value) > 0.01:
                            gray = np.dot(processed_img, [0.299, 0.587, 0.114]) / 255.0
                            # 根据值选择调整高光还是阴影
                            if value > 0:
                                mask = gray > 0.5  # 高光区域
                                amount = (gray[mask] - 0.5) * 2
                            else:
                                mask = gray < 0.5  # 阴影区域
                                amount = (0.5 - gray[mask]) * 2
                            
                            if np.any(mask):
                                factor = 1.0 + value * amount * 0.01
                                processed_img[mask] = processed_img[mask] * factor[:, np.newaxis]
